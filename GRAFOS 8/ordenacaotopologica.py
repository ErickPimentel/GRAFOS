...
class Grafo:
    ...
    def recursivedfs(self,v,vertices,aresta):
        lista=[]
        i=vertices.index(v)
        for a in range(len(vertices)):
            if aresta[i][a]>0:
                lista+=self.recursivedfs(vertices[a],vertices,aresta)
        ...
        for x in range(len(self.N)):
            for y in range(len(self.N)):
                if self.M[y][x]>0:
                    ...
                    for j in range(len(self.N)):
                        if self.M[x][j] > 0:
                            listaordenada+=self.recursivedfs(self.N[j],self.N,self.M)
                            for j in range(len(self.N)):
                                if self.M[j][j] > 0:
                                    self.M[j][j] = 0
                        if j==len(self.N)-1:
                            listaordenada += [self.N[x]]
