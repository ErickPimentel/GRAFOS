        ...
        while fim is False:
            for i in range(len(vertices)):
                for a in range(len(vertices)):
                    if vertices[i] == vez and ((aresta[i][a] != '-' and aresta[i][a] != 0) or (
                            aresta[a][i] != '-' and aresta[a][i] != 0)) and (vertices[a] in lista1) is False and (
                            ('{}-{}'.format(vertices[i], vertices[a]) in list_DFS) is False and (
                            '{}-{}'.format(vertices[a], vertices[i]) in list_DFS) is False):
                        lista1 += vertices[a]
                        vez = vertices[a]
                        list_DFS += ['{}-{}'.format(vertices[i], vertices[a])]
                    elif vertices[i] == vez and a == len(
                            vertices) - 1:
                        if vertices[i] == vez and ((aresta[i][a] != '-' and aresta[i][a] != 0) or (
                                aresta[a][i] != '-' and aresta[a][i] != 0)) and (vertices[a] in lista1) is True and (
                                ('{}-{}'.format(vertices[i], vertices[a]) in list_DFS) is False and (
                                '{}-{}'.format(vertices[a], vertices[i]) in list_DFS) is False) and (
                                ('{}-{}'.format(vertices[i], vertices[a]) in retorno) is False and (
                                '{}-{}'.format(vertices[a], vertices[i]) in retorno) is False):
                            retorno += ['{}-{}'.format(vertices[i], vertices[a])]
                            vez = lista1[lista1.index(vertices[i]) - 1]
                        else:
                            vez = lista1[lista1.index(vertices[i]) - 1]
                    elif vertices[i] == vez and ((aresta[i][a] != '-' and aresta[i][a] != 0) or (
                            aresta[a][i] != '-' and aresta[a][i] != 0)) and (vertices[a] in lista1) is True and (
                            ('{}-{}'.format(vertices[i], vertices[a]) in list_DFS) is False and (
                            '{}-{}'.format(vertices[a], vertices[i]) in list_DFS) is False) and (
                            ('{}-{}'.format(vertices[i], vertices[a]) in retorno) is False and (
                        ...
                        retorno += ['{}-{}'.format(vertices[i], vertices[a])]
                    elif vertices[i] == vez and vez == vertices[0] and len(lista1) == len(
                            vertices):  # verifica se todos os vertices já foram analisados
                        fim = True
