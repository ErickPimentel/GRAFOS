        ...
        while fim is False:
            for i in range(len(vertices)):
                for a in range(len(vertices)):
                    if vertices[i] == vez:
                        if arestas[i][a] > 0 and (
                                vertices[a] in visitados) == False:  # verifica se é uma aresta válida(DFS)
                        ...
                        elif arestas[i][a] > 0 and (vertices[a] in visitados) == True and (
                                '{}-{}'.format(vertices[i], vertices[a]) in dfs) == False and (
                                '{}-{}'.format(vertices[i], vertices[a]) in cruzada) == False and (
                                '{}-{}'.format(vertices[i], vertices[a]) in retorno) == False:
                        ...
                        elif a == len(vertices) - 1 and anterior[
                            vertices[i]] != 'nao':  # caso o vertice não tenha filho volta pro anterior
                            vez = anterior[vertices[i]]
                        elif vez == inicio2 and len(visitados) == len(
                                vertices):  # verifica se todos os vertices ja foram analisados
                            fim = True
                        elif a == len(
                                vertices) - 1 and vez == inicio2:  # caso volte pro vertice raiz e ainda resta aresta a ser analisada procura outra raiz
